...
describe('(Common) Functional module unit tests', () => {
  ...
  it('curry: will correctly curry 3 argument functions', () => {
    var fn = (left: number, mid: number, right: number) => (left + mid) * right
  ...
  it('compose: will compose functions without throwing', () => {
    expect(common.compose.bind(common.compose, helper.halve, helper.double)).to
      .not.throw
  ...
  it('compose: will throw arity errors when non-right-most is not unary', () => {
    expect(
      common.compose.bind(common.compose, Math.pow, helper.halve)
    ).to.throw(errors.AllFuncsMustBeUnary)

    expect(
      common.compose.bind(common.compose, helper.double, Math.pow, helper.halve)
    ).to.throw(errors.AllFuncsMustBeUnary)
  })

  it('compose: will throw insufficient args error', () => {
    expect(common.compose.bind(common.compose, Math.pow)).to.throw(
      errors.InsufficientValues
    )
  })

  it('compose: will throw invalid input error', () => {
    expect(common.compose.bind(common.compose, 1 as any, Math.pow)).to.throw(
      errors.AllMustBeFunctions
    )

    expect(common.compose.bind(common.compose, Math.pow, 1 as any)).to.throw(
      errors.AllMustBeFunctions
    )
  })

  it('compose: will correctly compose functions and return the expect value', () => {
    var c1 = common.compose(
      helper.double,
      Math.pow
    )
