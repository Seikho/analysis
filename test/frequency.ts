...
describe('Frequency tests', () => {
  ...
  it('histogram: will correctly throw if provided too many options', () => {
    ...
    expect(
      frequency.histogram.bind(frequency.histogram, histData, opts)
    ).to.throw(errors.HistogramOneOption)
  })
  ...
  it('histogram: will correctly create a histogram with min and max options provided', () => {
    ...
    let histTable = frequency.histogram(
      [1, 6, 6, 11, 11, 11, 16, 16, 16, 16, 21, 21, 21, 21, 21],
      binOptions
    )
