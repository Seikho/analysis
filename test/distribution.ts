...
describe('Distribution unit tests', () => {
  ...
  it('chiSquare: will throw if either value is not a number', () => {
    var fn = (v1: any, v2: any) => dist.chiSquare.bind(dist.chiSquare, v1, v2)
  ...
  it('binomialCoefficient: will throw when either argument is not a number', () => {
    expect(binCoeff.bind(binCoeff, <any>'string', 5)).to.throw(
      errors.MustBeNumber
    )
    expect(binCoeff.bind(binCoeff, 5, <any>'string')).to.throw(
      errors.MustBeNumber
    )
  })
  ...
  it("binomialCoefficient: will throw when 'events' argument is below 1", () => {
    expect(binCoeff.bind(binCoeff, 0, 5)).to.throw(
      errors.EventsMustBeAtLeastOne
    )
    expect(binCoeff.bind(binCoeff, -1, 5)).to.throw(
      errors.EventsMustBeAtLeastOne
    )
  })

  it("binomialCoefficient: will throw when 'x' argument is below 0", () => {
    expect(binCoeff.bind(binCoeff, 5, -1)).to.throw(
      errors.RandomVariableMustBeAtLeastZero
    )
  })
