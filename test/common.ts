...
describe('Common module unit tests', () => {
  ...
  it('isWhole: will throw if passed a non-number', () => {
    expect(common.isWhole.bind(common.isWhole, 'not a number' as any)).to.throw(
      errors.MustBeNumber
    )
  })
  ...
  it('isEven: will throw when invalid input is passed to isEven', () => {
    expect(common.isEven.bind(common.isEven, 'not a number' as any)).to.throw(
      errors.MustBeNumber
    )
  })
  ...
  it('toArray: will throw when invalid input', () => {
    expect(common.toArray.bind(common.toArray, 'bad type')).to.throw(
      errors.MustBeArrayOrObject
    )
    expect(common.toArray.bind(common.toArray, ['word', 1, 2, 3, 4])).to.throw(
      errors.AllMustBeNumbers
    )
  })
  ...
  it('factorial: will throw if supplied a non-number', () => {
    expect(common.factorial.bind(common.factorial, 'string' as any)).to.throw(
      errors.MustBeNumber
    )

    expect(common.factorial.bind(common.factorial, NaN)).to.throw(
      errors.MustBeNumber
    )
  })

  it('factorial: will throw if supplied non-whole number', () => {
    expect(common.factorial.bind(common.factorial, 2.1)).to.throw(
      errors.MustBeWhole
    )
  })

  it('factorial: will throw if given a number below zero', () => {
    expect(common.factorial.bind(common.factorial, -1)).to.throw(
      errors.MustBeAtLeastZero
    )
  })
})
